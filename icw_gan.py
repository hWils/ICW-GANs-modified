...
parser.add_argument("--or_shape", type=str, default="[79,95,69]", help="original image shape") # HW downscaled is [53,63,46] upscaled is [79, 95, 69],
...
parser.add_argument("--imageFile", type=str, default="original_dim.pkl", help="imageFile") # not necerssary for new load method
parser.add_argument("--labelFile", type=str, default="multi_class_pic_tags.pkl", help="labelFile")# not necerssary for new load method
...
parser.add_argument("--BATCH_SIZE", type=int, default="15", help="BATCH_SIZE") # HW: was originally 50
...
parser.add_argument("--ITERS", type=int, default="1010", help="How many generator iterations to train for") #HW: was originally 200,000: 5-10 epochs.
...
def generate_image(frame):
	...
	for i in range(BATCH_SIZE):
		...
		img = nibabel.Nifti1Image(samples_b[i, :, :, :], np.eye(4)) # HW: removed mask as not using one here msk.affine)
		plt.show()
		# zero-out
...
def save_test_img(frame):
	...
	for i in range(BATCH_SIZE):
		...
		print( "id  ", list(test_labels[count]), list(test_labels[count]).index(1.))
...
with tf.compat.v1.Session(config=sess_config) as session:
	...
	for iteration in xrange(ITERS): # How many generator iterations to train for
		...
		if (iteration % 10 == 0): #HW: was originally 1000
			generate_image(iteration)
		# HW: they dont have a python testing file, they use this to save images
		if (iteration > 20): #originally 20000
			save_test_img(iteration)
		if (iteration > 30): #originally 20500
			print('over')
			break
